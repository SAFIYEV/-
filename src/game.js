...
async function getOrCreateUser() {
    try {
        ...
        if (checkError) {
            ...
            if (checkError.message === 'Invalid API key') {
                console.error('Неверный ключ API. Проверьте настройки подключения к Supabase.');
            }
            return null;
        }
        ...
        if (createError) {
            ...
            if (createError.message === 'Invalid API key') {
                console.error('Неверный ключ API. Проверьте настройки подключения к Supabase.');
            }
            return null;
        }
    ...
    async load() {
        try {
            ...
            if (user) {
                ...
                // Если не удалось получить данные из БД, используем localStorage
                const localData = localStorage.getItem('zombieShooterData');
                if (localData) {
